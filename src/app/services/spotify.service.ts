  ...
  getCurrentlyPlaying(){
    ...
    return this.http.get<PlayingState>(url).pipe(map((data:PlayingState)=>{
        ...
        if(this.currentTrack?.id !== data?.item.id)
        {
          ...
          if(this.currentTrack != undefined)
            this.currentTrack.trackState = TrackState.undefined;
          
          if(data?.context?.uri && this.currentTrack!= undefined)
          {
            ...
            if(parts[1] == "playlist")
            {
              ...
              {
                if(this.currentTrack!=undefined)
                  this.currentTrack.trackState = inPlaylist?TrackState.AlreadyOnPlaylist:TrackState.NotOnPlaylist;
              })
            }
            else
              this.currentTrack.trackState = TrackState.NotPlayingPlaylist;
          }
          this.onChangeTrack.next({previousTrack:previousTrack, state:this.playState, currentTrack:this.currentTrack});
        }
